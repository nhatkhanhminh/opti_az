...
TOKEN_DATA.unknown = { symbol: "USDC", icon: "/images/tokens/usdc.webp", name: "USDC", price: 1 };
const AZC_PRICE = 2;
...
function StakingCard({ 
  ...
  onResetClaim?: (stakeId: bigint) => void
}) {
  ...
  // Thêm state để prevent double click
  ...
  // }
  // Hàm xử lý claim riêng cho từng card với double-click prevention
  const handleLocalClaim = async () => {
    if (isClicking || isPendingClaim) {
          ...
          {isPendingClaim && isClaimingId?.toString() === stakingData.id.toString() ? (
            ...
            <div className="flex items-center gap-2">
              <RainbowButton
                ...
                disabled={isPendingClaim || isClicking}
              >
                <span>{isClicking ? "Processing..." : t('buttons.claimRewards')}</span>
              </RainbowButton>
              
              {/* Thêm nút reset nếu card này đang trong trạng thái claiming */}
              {isClaimingId?.toString() === stakingData.id.toString() && onResetClaim && (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => onResetClaim(stakingData.id)}
...
export default function MyStaking() {
  ...
  // Thêm state để track pending timeout
  const [pendingTimeouts, setPendingTimeouts] = useState<Record<string, NodeJS.Timeout>>({});
  
  // Thêm state để force refresh
  ...
  // Thêm forceRefreshKey vào dependency để trigger refresh
  ...
  // Refresh data khi forceRefreshKey thay đổi
  useEffect(() => {
    // Force component re-render when forceRefreshKey changes
  }, [forceRefreshKey]);
  ...
  // Cải thiện effect để handle success/error với timeout fallback
  useEffect(() => {
    if (isErrorClaim && claimingStakeId) {
      // Reset immediately on error
      setClaimingStakeId(null);
      // Reset transaction active state
      if ((window as any).setTransactionActive) {
        (window as any).setTransactionActive(false);
      }
      // Clear any pending timeout
      const stakeIdStr = claimingStakeId.toString();
      if (pendingTimeouts[stakeIdStr]) {
        clearTimeout(pendingTimeouts[stakeIdStr]);
        setPendingTimeouts(prev => {
          const newTimeouts = { ...prev };
          delete newTimeouts[stakeIdStr];
          return newTimeouts;
        });
      }
    }
    ...
    if (isSuccessClaim && claimingStakeId) {
      ...
      // Force refresh blockchain data after success
      setTimeout(() => {
      ...
      const timer = setTimeout(() => {
        setClaimingStakeId(null);
        // Reset transaction active state sau khi hoàn thành
        if ((window as any).setTransactionActive) {
          (window as any).setTransactionActive(false);
        }
      }, 3500);
  ...
  }, [isSuccessClaim, isErrorClaim, claimingStakeId, pendingTimeouts]);

  // Thêm effect để handle timeout cho pending transactions
  useEffect(() => {
    if (isPendingClaim && claimingStakeId) {
      const stakeIdStr = claimingStakeId.toString();
      
      // Set timeout để force reset nếu transaction pending quá lâu (30 giây)
      const timeoutId = setTimeout(() => {
        console.warn(`Claim timeout for stake ${stakeIdStr}, resetting state`);
        setClaimingStakeId(null);
        // Reset transaction active state on timeout
        if ((window as any).setTransactionActive) {
          (window as any).setTransactionActive(false);
        }
        toast.warning("Transaction taking too long. Please check your transaction status and try again if needed.");
      ...
      setPendingTimeouts(prev => ({
        ...prev,
        [stakeIdStr]: timeoutId
      }));

      // Cleanup on unmount or when isPendingClaim changes
      return () => {
  ...
  }, [isPendingClaim, claimingStakeId]);

  // Thêm function để force refresh
  const forceRefresh = useCallback(() => {
    setForceRefreshKey(prev => prev + 1);
    // Reset all claiming states
    setClaimingStakeId(null);
    // Reset transaction active state
    if ((window as any).setTransactionActive) {
      (window as any).setTransactionActive(false);
    ...
    // Clear all timeouts
    Object.values(pendingTimeouts).forEach(timeout => clearTimeout(timeout));
    setPendingTimeouts({});
    toast.info("Refreshing data...");
  }, [pendingTimeouts]);

  // Thêm function để reset stuck claim state
  const resetClaimState = useCallback((stakeId: bigint) => {
    setClaimingStakeId(null);
    // Reset transaction active state
    if ((window as any).setTransactionActive) {
      (window as any).setTransactionActive(false);
    }
    const stakeIdStr = stakeId.toString();
    if (pendingTimeouts[stakeIdStr]) {
      clearTimeout(pendingTimeouts[stakeIdStr]);
      setPendingTimeouts(prev => {
        const newTimeouts = { ...prev };
    ...
    toast.info("Claim state reset. You can try again.");
  }, [pendingTimeouts]);
  ...
  const handleClaim = useCallback(async (stakeId: bigint) => {
    ...
    // Kiểm tra nếu đang có claim khác đang pending
    if (claimingStakeId && claimingStakeId !== stakeId) {
      toast.warning("Please wait for the current claim to complete");
      return;
    }
    
    // Set transaction active state để tránh wallet event conflicts
    if ((window as any).setTransactionActive) {
      (window as any).setTransactionActive(true);
    }
    
    // Lưu stakeId đang được claim
    setClaimingStakeId(stakeId);

    // Tìm stake trong danh sách userStakes
    ...
    // Kiểm tra usdtAmount để xác định sử dụng contract nào
    const usdtAmountInUsd = Number(stake.usdtAmount) / 1e18;
    const useSmallContract = usdtAmountInUsd < 500;
    

    // use main claim contract
    ...
    try {
      ...
      // Nếu có lỗi, cũng reset claimingStakeId và transaction state
      setClaimingStakeId(null);
      if ((window as any).setTransactionActive) {
        (window as any).setTransactionActive(false);
      }
  ...
  }, [account, contractClaim, claimReward, userStakes, claimingStakeId]);
          ...
          <div className="flex gap-2 sm:ml-auto">
            ...
            {claimingStakeId && (
              <Button
                variant="outline"
                size="sm"
                onClick={() => resetClaimState(claimingStakeId)}
